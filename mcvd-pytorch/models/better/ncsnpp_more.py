...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    modules.append(
        layerspp.get_act_norm(act=act, act_emb=act, norm='group', ch=in_ch, is3d=self.is3d, n_frames=self.num_frames))
...
class SPADE_NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    modules.append(
        layerspp.get_act_norm(act=act, act_emb=act, norm='spade', ch=in_ch, is3d=self.is3d, n_frames=self.num_frames, num_frames_cond=num_frames_cond,
